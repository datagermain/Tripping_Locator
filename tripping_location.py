...
seg_uv   = []   # (u, v) endpoints for each segment
...
def insert_snapped_node(Gbase: nx.Graph, pt_tm: Point):
    ...
    pnode = snap_xy(p_on.x, p_on.y, SNAP_TOL)
    off_m = float(pt_tm.distance(Point(pnode)))

    # if very close to an endpoint, reuse it
    if Point(pnode).distance(Point(u)) < 0.5:
        return Gbase, u, off_m
    if Point(pnode).distance(Point(v)) < 0.5:
        return Gbase, v, off_m

    Gnew = Gbase.copy()
    if Gnew.has_edge(u, v):
        Gnew.remove_edge(u, v)

    up_len = float(LineString([u, pnode]).length)
    pv_len = float(LineString([pnode, v]).length)

    Gnew.add_node(u); Gnew.add_node(v); Gnew.add_node(pnode)
    Gnew.add_edge(u, pnode, weight=up_len)
    Gnew.add_edge(pnode, v, weight=pv_len)

    return Gnew, pnode, off_m
...
if btn and src_name != dst_name:
    ...
    try:
        ...
        pts_tm_gdf = gpd.GeoDataFrame(
            geometry=[Point(x, y) for x, y in path_nodes], crs=RWANDA_TM30
        ).to_crs(4326)
        ...
        path_tm = LineString(path_nodes)  # TM meters
